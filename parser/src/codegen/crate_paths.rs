...
use shared::{
    syn::{
        error::{CriticalResult, GetVec, StrToError},
        vec_to_path,
    },
...
impl Crates {
    pub fn new(crates: Vec<AstCrate>, crate_idxs: [usize; Crate::LEN]) -> CriticalResult<Self> {
    ...
    fn find_path(
        ...
        end_idx: usize,
        crates: &Vec<AstCrate>,
    ) -> CriticalResult<Option<Vec<String>>> {
        // Base case
    ...
    ) -> CriticalResult<Vec<(usize, Vec<String>)>> {
    ...
    pub fn get_crate_syn_paths<const N: usize>(
        ...
        cr_idx: usize,
        block_crates: [usize; N],
    ) -> CriticalResult<Vec<(usize, syn::Path)>> {
    ...
    pub fn get_crate_syn_path(
        &self,
        start_idx: usize,
        end_idx: usize,
    ) -> CriticalResult<syn::Path> {
    ...
    pub fn get_named_crate_syn_path(
        &self,
        start_idx: usize,
        cr: Crate,
    ) -> CriticalResult<syn::Path> {
    ...
    pub fn get_item_path(&self, start_idx: usize, path: &ItemPath) -> CriticalResult<Vec<String>> {
    ...
    pub fn get_path(&self, start_idx: usize, item: &CratePath) -> CriticalResult<Vec<String>> {
        self.get_item_path(
            start_idx,
    ...
    pub fn get_syn_path(&self, start_idx: usize, item: &CratePath) -> CriticalResult<syn::Path> {
    ...
    pub fn get_crate<'a>(&'a self, cr: Crate) -> CriticalResult<&'a AstCrate> {
        self.crates.try_get(self.get_crate_index(cr))
    }

    pub fn get_crate_mut<'a>(&'a mut self, cr: Crate) -> CriticalResult<&'a mut AstCrate> {
...
impl GetVec<AstCrate> for Crates {
    fn try_get<'a>(&'a self, i: usize) -> CriticalResult<&'a AstCrate> {
        self.crates.try_get(i)
    }

    fn try_get_mut<'a>(&'a mut self, i: usize) -> CriticalResult<&'a mut AstCrate> {
