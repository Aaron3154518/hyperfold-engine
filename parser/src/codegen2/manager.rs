...
pub fn manager_impl(cr_idx: usize, items: &Items, crates: &Crates) -> MsgsResult<TokenStream> {
    ...
    match_ok!(Zip2Msgs, component_set_fns, result, {
        ...
        quote!(
            impl #manager {
                ...
                pub fn run(&mut self) {
                    ...
                    let mut t = unsafe { hyperfold_engine::sdl2::SDL_GetTicks() };
                    let mut dt;
                    let mut tsum: u64 = 0;
                    let mut tcnt: u64 = 0;
                    while !self.#globals_var.g1_5.quit {
                        dt = unsafe { hyperfold_engine::sdl2::SDL_GetTicks() } - t;
                        t += dt;
                        self.tick(dt);
                        dt = unsafe { hyperfold_engine::sdl2::SDL_GetTicks() } - t;
                        tsum += dt as u64;
                        tcnt += 1;
                        if dt < FRAME_TIME {
                            unsafe { hyperfold_engine::sdl2::SDL_Delay(FRAME_TIME - dt) };
                        }
                    }
                ...
                fn init_events(&self, ts: u32) -> #events {
                    ...
                    hyperfold_engine::ecs::events::AddEvent::new_event(
                        &mut #events_var,
                        hyperfold_engine::ecs::events::core::Events,
                    );
                    hyperfold_engine::ecs::events::AddEvent::new_event(
                        &mut #events_var,
                        hyperfold_engine::ecs::events::core::Update(ts),
                    );
                    hyperfold_engine::ecs::events::AddEvent::new_event(
                        &mut #events_var,
                        hyperfold_engine::ecs::events::core::PreRender,
                    );
                    hyperfold_engine::ecs::events::AddEvent::new_event(
                        &mut #events_var,
                        hyperfold_engine::ecs::events::core::Render,
                    );
                    #events_var
                }
